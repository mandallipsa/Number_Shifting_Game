...
void createMatrix(int arr[][4])
{
    ...
    for(i=0;i<4;i++)
       for(j = 0;j<4;j++)
       {
         if(lastIndex >= 0)
         {
            index = rand() % (lastIndex + 1);//idea :perforaming % operation by (lastIndex + 1)
...
void gameRule(int arr[][4])
{
  ...
  printf(ANSI_COLOR_RED "\n1.You can move only 1step at a time by arrow key " ANSI_COLOR_RESET);
  ...
  printf(ANSI_COLOR_RED "\n4. Winning situation : " ANSI_COLOR_RESET);
  ...
  printf(ANSI_COLOR_YELLOW "---------------------------\n" ANSI_COLOR_RESET);
  printf("\n5.You can exit from game at any time by pressing 'E' or 'e' ");
  ...
  while (1)
  {
     ...
     while (!winner(arr)) //checking for winning situation
     {
       ...
       switch (key) //mapping
       {
        ...
        case 69: //ascii of e
           printf("\a\a\a\a\a\a\n   THANKS FOR PLAYING ! \n\a");
           printf("\nHit 'Enter' to exit from the game \n");
           key = readEnteredKey();
           return 0;
      ...
      if(!maxTry)
         ...
         printf(ANSI_COLOR_GREEN "\n\a!!!!!!!!!!!!!!Congratuations %s for winning this game!!!!!!!!!!!!!!!\a\n"ANSI_COLOR_RESET);
